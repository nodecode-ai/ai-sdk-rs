...
impl<T: HttpTransport + Send + Sync + 'static> LanguageModel for GoogleGenAiLanguageModel<T> {
    ...
    async fn do_stream(&self, options: v2t::CallOptions) -> Result<StreamResponse, SdkError> {
        ...
        let stream: PartStream = Box::pin(async_stream::try_stream! {
            ...
            let mut last_code_tool_id: Option<String> = None;
            let mut emitted_source_urls: std::collections::HashSet<String> = std::collections::HashSet::new();
                ...
                ($ev:expr) => {{
                    ...
                    if let Some(cands) = parsed.get("candidates").and_then(|v| v.as_array()) {
                        for cand in cands {
                                ...
                                for p in parts {
                                    ...
                                    if let Some(txt) = p.get("text").and_then(|v| v.as_str()) {
                                        if !txt.is_empty() {
                                            let is_thought = p.get("thought").and_then(|v| v.as_bool()).unwrap_or(false);
                                            let thought_sig = p.get("thoughtSignature").and_then(|v| v.as_str());
                                            let pm = thought_sig.map(|sig| { let mut outer = HashMap::new(); let mut inner = HashMap::new(); inner.insert("thoughtSignature".into(), JsonValue::String(sig.to_string())); outer.insert("google".into(), inner); outer });
                                            if is_thought {
                            ...
                            // finish reason and provider metadata update
                            if let Some(fr) = cand.get("finishReason").and_then(|v| v.as_str()) {
                                ...
                                let mut outer = HashMap::new(); outer.insert("google".into(), inner_map); provider_metadata = Some(outer);
                            }
                        }
                    }
                }};
