...
/// Convert JSON Schema (draft-07) into an OpenAPI 3.0-ish schema expected by Google GenAI.
/// This mirrors the limited conversion used by the Vercel AI SDK.
pub fn convert_json_schema_to_openapi_schema(schema: &JsonValue) -> JsonValue {
    ...
    if is_empty_object_schema(schema) {
        return JsonValue::Null; // caller should skip Null
    }
    match schema {
        ...
        JsonValue::Object(map) => {
            ...
            if let Some(any) = map.get("anyOf").and_then(|v| v.as_array()) {
                // if contains null, mark nullable and keep non-null entries
                let has_null = any.iter().any(|s| matches!(s, JsonValue::Object(m) if m.get("type").and_then(|v| v.as_str()) == Some("null")));
                let non_null: Vec<JsonValue> = any.iter().filter(|s| !matches!(s, JsonValue::Object(m) if m.get("type").and_then(|v| v.as_str()) == Some("null"))).map(convert_json_schema_to_openapi_schema).collect();
                if has_null {
                    if non_null.len() == 1 {
                        // single schema + nullable
...
/// Prepare Google GenAI tools payloads (function and provider-defined tools).
